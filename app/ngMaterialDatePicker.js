(function () {
  'use strict';
  let moduleName = "ngMaterialDatePicker";
  ...
  let css = function (el, name) {
    el = angular.element(el);
    return ('getComputedStyle' in window) ? window.getComputedStyle(el[0])[name] : el.css(name);
  };

  let template = '<md-dialog class="dtp" layout="column" style="width: 300px;">'
  ...
  angular.module(moduleName, ['ngMaterial'])
    ...
    */
    .factory('mdcDateTimeDialog', ["$mdDialog", "$q", "mdcDefaultParams", function ($mdDialog, $q, mdcDefaultParams) {
      let accepted_options = Object.keys(mdcDefaultParams);
      console.log(accepted_options);
      let service = {
        show: function (options) {
          let deferred = $q.defer();
          let params = angular.copy(mdcDefaultParams);
  ...
  let PluginController = function ($scope, $mdDialog, mdcDefaultParams) {
  ...
  PluginController.prototype = {
    ...
    currentNearest5Minute: function () {
      let date = this.currentDate || moment();
      let minutes = (5 * Math.round(date.minute() / 5));
      if (minutes >= 60) {
    ...
    initDates: function () {
      let that = this;
      let _dateParam = function (input, fallback) {
        let ret = null;
    ...
    isInDisableDates: function (date) {
      let dut = date.format('MMMM Do YYYY')
  ...
  angular.module(moduleName)
    .directive('mdcDatetimePickerCalendar', [
      function () {
        ...
        let YEAR_MIN = 1920,
          ...
          ITEM_HEIGHT = 240,
          MONTHS = [];
        for (let i = 0; i < MONTHS_IN_ALL; i++) {
          MONTHS.push(i);
        }

        let currentMonthIndex = function (date, low) {
          low = low ? low : 0;
          let year = date.year();
        ...
        return {
          ...
          controller: ['$scope', function ($scope) {
            this.$onInit = function () {
              ...
              for (let i = picker.params.weekStart; days.length < 7; i++) {
                if (i > 6) {
                  i = 0;
                }
              ...
              if (!picker.maxDate && !picker.minDate) {
                ...
                let low = picker.minDate ? currentMonthIndex(picker.minDate) : 0;
                let high = picker.maxDate ? (currentMonthIndex(picker.maxDate) + 1) : MONTHS_IN_ALL;
                calendar.months = MONTHS.slice(low, high);
              }


              calendar.getItemAtIndex = function (index) {
                let month = ((index + 1) % 12) || 12;
                let year = YEAR_MIN + Math.floor(index / 12);
                let monthObj = moment(picker.currentDate)
                  .year(year)
                  .month(month);
              ...
              $scope.$watch(function () {
                ...
                if (val2 != val1) {
                  let nDate = moment(val2, 'YYYY-MM');
                  let low = picker.minDate ? currentMonthIndex(picker.minDate) : 0;
                  let index = currentMonthIndex(nDate, low);
              ...
              let generateMonthCalendar = function (date) {
                let month = {};
                if (date !== null) {
                  month.name = date.format('MMMM YYYY');
                  let startOfMonth = moment(date).locale(picker.params.lang).startOf('month')
                    .hour(date.hour())
                    .minute(date.minute())
                  ;
                  let iNumDay = startOfMonth.format('d');
                  month.days = [];
                  for (let i = startOfMonth.date(); i <= startOfMonth.daysInMonth(); i++) {
                    if (i === startOfMonth.date()) {
                      let iWeek = calendar.week.indexOf(iNumDay.toString());
                      if (iWeek > 0) {
                  ...
                  let daysInAWeek = 7, daysTmp = [], slices = Math.ceil(month.days.length / daysInAWeek);
              ...
              calendar.isDateOfTheDay = function (m) {
                let today = calendar.picker.options.showTodaysDate;
                if (!today) {
                  return false;
                }
    ...
    .directive('mdcDatetimePickerCalendarMonth', ['$compile',
      function ($compile) {
        let buildCalendarContent = function (element, scope) {
          let tbody = angular.element(element[0].querySelector('tbody'));
          let calendar = scope.cal, month = scope.month;
  ...
  angular.module(moduleName)
    .directive('mdcDatetimePickerClock', [
      function () {
        ...
        let template = '<div class="dtp-picker-clock"><span ng-if="!points || points.length < 1">&nbsp;</span>'
        ...
        return {
          ...
          link: function (scope, element, attrs) {
            let minuteMode = attrs.mode === 'minutes';
            let picker = scope.picker;
            //banking on the fact that there will only be one at a time
            let componentRoot = document.querySelector('md-dialog.dtp');
            let exec = function () {
              let clock = angular.element(element[0].querySelector('.dtp-picker-clock')),
                pickerEl = angular.element(componentRoot.querySelector('.dtp-picker'));

              let w = componentRoot.querySelector('.dtp-content').offsetWidth;
              let pl = parseInt(css(pickerEl, 'paddingLeft').replace('px', '')) || 0;
              let pr = parseInt(css(pickerEl, 'paddingRight').replace('px', '')) || 0;
              let ml = parseInt(css(clock, 'marginLeft').replace('px', '')) || 0;
              let mr = parseInt(css(clock, 'marginRight').replace('px', '')) || 0;
              //set width
              let clockWidth = (w - (ml + mr + pl + pr));
              clock.css('width', (clockWidth) + 'px');

              let pL = parseInt(css(pickerEl, 'paddingLeft').replace('px', '')) || 0;
              let pT = parseInt(css(pickerEl, 'paddingTop').replace('px', '')) || 0;
              let mL = parseInt(css(clock, 'marginLeft').replace('px', '')) || 0;
              let mT = parseInt(css(clock, 'marginTop').replace('px', '')) || 0;
              ...
              for (let h = 0; h < 12; ++h) {
                let x = j * Math.sin(Math.PI * 2 * (h / 12));
                let y = j * Math.cos(Math.PI * 2 * (h / 12));
                let left = (r + x + pL / 2) - (pL + mL);
                let top = (r - y - mT / 2) - (pT + mT);
              ...
              let clockCenter = element[0].querySelector('.dtp-clock-center');
              let centerWidth = (clockCenter.offsetWidth / 2) || 7.5,
            ...
            let setCurrentValue = function () {
              let date = picker.currentNearest5Minute();
              scope.currentValue = minuteMode ? date.minute() : (date.hour() % 12);
            };

            scope.$watch(function () {
              let tmp = picker.currentNearest5Minute();
            ...
            let setDisplayPoints = function (isPM, points) {
            ...
            if (!picker.params.shortTime) {
              scope.$watch('picker.meridien', function () {
                if (!minuteMode) {
                  if (scope.points) {
                    let points = setDisplayPoints(picker.isPM(), angular.copy(scope.points));
                    scope.points = points;
                  }
                }
              });
            ...
            let unWatcher = scope.$watch(function () {
